...
from wexample_helpers.helpers.file import file_get_dir_size, file_get_human_readable_size
...
def system__dir__spaces(context: "ExecutionContext", path: Optional[str] = None):
    ...
    entries = sorted(base.iterdir(), key=lambda e: (
        file_get_dir_size(e) if e.is_dir() else (e.stat().st_size if e.is_file() and not e.is_symlink() else 0)
    ))
