...
class AddonCommandResolver(AbstractCommandResolver):
    def autocomplete_suggest(self, cursor: int, search_split: list[str]) -> str | None:
        ...
        all_addon_cmds = [
            cmd["command"]
            for addon in addon_data.values()
        ...
        if cursor == 0:
            addon_names = sorted(set(c.split("::")[0] for c in all_addon_cmds if "::" in c))
            suggestions = [f"{a}{COMMAND_SEPARATOR_ADDON}" for a in addon_names if a.startswith(first)]
