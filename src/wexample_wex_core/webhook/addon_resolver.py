...
class AddonWebhookTypeResolver:
    """Resolve webhook command and token for addon-type URLs.

    URL format: /webhook/addon/{addon_name}/{command/path}
    Command:    {addon_name}::{command/path}
    Token file: {wex_workdir}/.wex/local/webhook_tokens_addon.yml
    """
    ...
    def resolve_token(self, command_path: str, command_str: str) -> str | None:
        ...
        token_file = (
            self._workdir / WORKDIR_SETUP_DIR / WORKDIR_LOCAL_DIR_NAME / "webhook_tokens_addon.yml"
        )
