...
class TestRegistryHelpers(AbstractKernelTest):
    def test_find_command_by_alias(self, kernel) -> None:
        cmd = kernel.get_configuration_registry().find_command("ping")
        assert cmd is not None
        assert cmd["command"] == "demo::ping/pong"

    def test_find_command_by_name(self, kernel) -> None:
        cmd = kernel.get_configuration_registry().find_command("demo::ping/pong")
        assert cmd is not None
        assert cmd["command"] == "demo::ping/pong"

    def test_find_command_unknown_returns_none(self, kernel) -> None:
        assert (
            kernel.get_configuration_registry().find_command("does::not/exist") is None
        )

    def test_get_all_command_names_includes_aliases(self, kernel) -> None:
        names = kernel.get_configuration_registry().get_all_command_names()
        assert "demo::ping/pong" in names
        assert "ping" in names  # alias

    def test_get_all_commands_keyed_by_command_name(self, kernel) -> None:
    ...
    def test_get_all_commands_returns_flat_dict(self, kernel) -> None:
        commands = kernel.get_configuration_registry().get_all_commands()
        assert isinstance(commands, dict)
        assert len(commands) > 0
        assert all("command" in data for data in commands.values())

    def test_get_sudo_commands(self, kernel) -> None:
    ...
    def test_get_sudo_commands_excludes_non_sudo(self, kernel) -> None:
        sudo_commands = kernel.get_configuration_registry().get_sudo_commands()
        assert "demo::ping/pong" not in sudo_commands

    def test_suggest_alias_by_prefix(self, kernel) -> None:
        results = kernel.get_configuration_registry().suggest("pi")
        assert "ping" in results

    def test_suggest_by_prefix(self, kernel) -> None:
