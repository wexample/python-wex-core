...
class TestResponseCollectionResponse(AbstractResponseTest):
    ...
    def test_json_contains_all_items(self, kernel) -> None:
    ...
    def test_ping_collection_contains_two_items(self, kernel) -> None:
    ...
    def test_ping_returns_collection_response(self, kernel) -> None:
        response = kernel.execute_kernel_command(
            self._make_request(kernel, output_target=[OUTPUT_TARGET_NONE])
        )
