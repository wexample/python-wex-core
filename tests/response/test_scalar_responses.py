...
class TestStrResponse(AbstractKernelTest):
    def test_str_format(self, kernel):
        assert StrResponse(kernel=kernel, content="hello").get_formatted(OUTPUT_FORMAT_STR) == "hello"

    def test_get_printable(self, kernel):
        assert StrResponse(kernel=kernel, content="hello").get_printable() == "hello"

    def test_rejects_non_str(self, kernel):
