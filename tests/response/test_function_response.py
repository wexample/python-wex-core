...
from wexample_wex_core.addons.demo.commands.ping.pong import (
    PING_TYPE_FUNCTION,
    demo__ping__pong,
)
...
class TestFunctionResponse(AbstractKernelTest):
    def test_cached_execution(self, kernel) -> None:
        call_count = 0

        def counted() -> DictResponse:
            nonlocal call_count
            call_count += 1
    ...
    def test_executes_callable(self, kernel) -> None:
    ...
    def test_via_ping_command(self, kernel) -> None:
